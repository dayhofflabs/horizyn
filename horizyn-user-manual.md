...
**Multi-Label Retrieval**: Each reaction (query) can be catalyzed by multiple proteins (targets). For example, the SOTA dataset has ~68K test pairs (after bidirectional augmentation) representing ~2K unique reaction-directions with valid proteins per reaction direction.
...
**Example**: If query "rxn_123" has 3 valid proteins ["prot_A", "prot_B", "prot_C"], the top-1 metric checks if ANY of these 3 proteins ranks #1 out of all ~200K proteins.

**Why Three Dataloaders?**

This design separates three distinct validation tasks:
1. **Loss computation** - Uses pairs (~34K pairs in SOTA) to measure generalization
2. **Lookup table** - Loads all target embeddings once for efficiency
3. **Retrieval metrics** - Uses unique queries (~2K queries in SOTA) for multi-label evaluation
...
1. Computing loss on all pairs (~34K pairs in SOTA)
2. Building a lookup table of all target embeddings (~200K proteins in SOTA)
3. Evaluating retrieval metrics on unique queries (~2K reactions in SOTA)
...
**Full Screening Set**: The test lookup table contains ALL proteins from both training and test sets (~200K proteins). This ensures test queries can retrieve their target proteins even if those proteins only appear in test pairs.
